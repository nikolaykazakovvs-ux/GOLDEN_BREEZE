...
# 16. Каталог стратегий и мини-ТЗ для интеграции

Файл `configs/strategies_catalog.yaml` содержит список внешних стратегий и индикаторов, которые нужно постепенно внедрять в Golden Breeze Hybrid AI Core. Для каждого элемента из `strategies[*]` действует единое ТЗ:

> **TASK: integrate `<strategy.id>` features**
>
> **GOAL:**
> 1. Перейти по `source.url`, выбрать конкретный скрипт/репозиторий, максимально соответствующий описанию стратегии.
> 2. Создать адаптер `features_<strategy.id>.py`, который принимает `pandas.DataFrame` с OHLCV (и при необходимости сессию/тик-данные) и добавляет перечисленные в `outputs` поля. Никакой логики ордеров — только вычисление фичей/сигналов.
> 3. Подключить новые признаки в `aimodule/training/prepare_direction_dataset.py` (как дополнительные колонки) и, при необходимости, в `strategy/regime_strategies.py` или Intrabar Engine.
> 4. Обновить документацию (README/REPORTS), коротко описав, какие стратегии интегрированы и где они используются.
>
> **FILES TO MODIFY:**
> - `configs/strategies_catalog.yaml` (только при изменении каталога)
> - `tools/features_<strategy.id>.py` (новые адаптеры)
> - `aimodule/training/prepare_direction_dataset.py`
> - `strategy/regime_strategies.py` / `strategy/intrabar_engine.py` (по необходимости)
> - Документация (`README.md`, `REPORTS/*.md`)
>
> **ACCEPTANCE CRITERIA:**
> - Новые признаки корректно считаются и сохраняются в датасетах.
> - Обновлённые стратегии задействованы в соответствующих модулях.
> - Документация отражает изменения.
> - Тесты и линтеры проходят.

Этот блок можно копировать при постановке задач мультиагентной команде.
